...
/**
 * JdbcPagingItemReader / JdbcBatchItemWriter 를 사용해보는 종합 예제
 * ./gradlew bootRun --args='--spring.batch.job.name=orderRecoveryJob'
 */
@Configuration
...
public class OrderRecoveryJobConfig {
    ...
    @Bean
    public Job orderRecoveryJob(Step orderRecoveryStep) {
    ...
    @Bean
    public Step orderRecoveryStep(ItemReader<HackedOrder> compromisedOrderReader,
                                  ItemProcessor<HackedOrder, HackedOrder> orderStatusProcessor,
                                  ItemWriter<HackedOrder> orderStatusWriter) {
    ...
    @Bean
    public JdbcPagingItemReader<HackedOrder> compromisedOrderReader() throws Exception {
