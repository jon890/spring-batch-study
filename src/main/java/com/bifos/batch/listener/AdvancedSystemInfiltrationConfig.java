...
public class AdvancedSystemInfiltrationConfig {
    ...
    @Bean(JobNameConst.LISTENER_SYSTEM_INFILTRATION_JOB)
    public Job systemInfiltrationJob(JobRepository jobRepository,
                                     @Qualifier(JobNameConst.LISTENER_SYSTEM_INFILTRATION_RECON_STEP) Step reconStep,
                                     @Qualifier(JobNameConst.LISTENER_SYSTEM_INFILTRATION_ATTACK_STEP) Step attackStep) {
        return new JobBuilder(JobNameConst.LISTENER_SYSTEM_INFILTRATION_JOB, jobRepository)
    ...
    @Bean(JobNameConst.LISTENER_SYSTEM_INFILTRATION_RECON_STEP)
    public Step reconStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder(JobNameConst.LISTENER_SYSTEM_INFILTRATION_RECON_STEP, jobRepository)
    ...
    @Bean(JobNameConst.LISTENER_SYSTEM_INFILTRATION_ATTACK_STEP)
    public Step attackStep(JobRepository jobRepository,
                           PlatformTransactionManager transactionManager,
                           Tasklet attackStepTasklet) {
        return new StepBuilder(JobNameConst.LISTENER_SYSTEM_INFILTRATION_ATTACK_STEP, jobRepository)
