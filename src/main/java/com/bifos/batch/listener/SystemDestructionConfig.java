...
/**
 * Listener에서 JobParameters에 접근하는 예제
 * ./gradlew bootRun --args='--spring.batch.job.name=listener_systemDestruction_job terminationType=test'
 */
@Slf4j
@Configuration
public class SystemDestructionConfig {

    @Bean(JobNameConst.LISTENER_SYSTEM_DESTRUCTION_JOB)
    public Job killDashNineJob(JobRepository jobRepository,
                                  @Qualifier(JobNameConst.LISTENER_SYSTEM_DESTRUCTION_TERMINATION_STEP) Step terminationStep) {
        return new JobBuilder(JobNameConst.LISTENER_SYSTEM_DESTRUCTION_JOB, jobRepository)
    ...
    @Bean(JobNameConst.LISTENER_SYSTEM_DESTRUCTION_TERMINATION_STEP)
    public Step terminationStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder(JobNameConst.LISTENER_SYSTEM_DESTRUCTION_JOB, jobRepository)
